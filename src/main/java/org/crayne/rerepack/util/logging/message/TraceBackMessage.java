...
public class TraceBackMessage extends AbstractLoggingMessage {
    ...
    public void printTo(@NotNull final Logger logger) {
        final int line = at.line(), column = at.column();
        final File file = at.file();
        ...
        final String positionInformation = "at line " + line + ", " + "column " + column;
        final String fileInformation = file != null ? " in file " + file.getAbsolutePath() : "";

        logger.log(positionInformation + fileInformation, level());
        logger.log(message(), level());
